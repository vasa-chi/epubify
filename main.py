...
def export_book(site, page, cover=None):
    ...
    def clean_page(page_text):
        ...
        noinclude = re.compile(
            r'<p>&lt;noinclude&gt;.+?&lt;/noinclude&gt;.+?</p>', re.DOTALL)
        noinclude_safe = re.compile(
            r'&lt;noinclude&gt;.+?&lt;/noinclude&gt;.+?</p>', re.DOTALL)
        tr_note = re.compile(
            r'<h2\ id\=\"w\_translator.+?references\ group=.+?</p>', re.DOTALL)
    ...
    pages.extend([gevent.spawn(save_page, book_page) for book_page in
                  (job.value for job in jobs[1:])])
