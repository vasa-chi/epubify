...
class EpubBook:
    ...
    def getAllItems(self):
        return sorted(
            itertools.chain(self.imageItems.values(), self.htmlItems.values(),
                            self.cssItems.values()), key=lambda x: x.id)
    ...
    def __makeTitlePage(self):
        ...
        self.titlePage.html = stream.render('xhtml', doctype='xhtml11',
                                            drop_xml_decl=False)
    ...
    def __makeTocPage(self):
        ...
        stream = tmpl.generate(book=self)
        self.tocPage.html = stream.render('xhtml', doctype='xhtml11',
                                          drop_xml_decl=False)
    ...
    def addSpineItem(self, item, linear=True, order=None):
        ...
        if order == None:
            order = (max(
                order for order, _, _ in self.spine) if self.spine else 0) + 1
    ...
    def __writeContainerXML(self):
        fout = open(os.path.join(self.rootDir, 'META-INF', 'container.xml'),
                    'w')
    ...
    def __writeItems(self):
        for item in self.getAllItems():
            ...
            if item.html:
                fout = open(os.path.join(self.rootDir, 'OEBPS', item.destPath),
                            'w')
                fout.write(item.html)
                fout.close()
            else:
                shutil.copyfile(item.srcPath,
                                os.path.join(self.rootDir, 'OEBPS',
                                             item.destPath))
